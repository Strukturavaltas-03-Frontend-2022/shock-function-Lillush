...
const getActionPrice = function (price) {
  return Math.round(price * 1.35 * 0.9);
};
...
const getProductDescrition = (product) => {
  const productName = product.name[0].toUpperCase() + product.name.slice(1);
  const price = Math.round(product.price * 1.35);
  return `${productName}, $${price}, in stock: ${product.stock}`;
};
...
const getCarDetails = (
  ...
  return `${car.manufacturer[0].toUpperCase() + car.manufacturer.slice(1)}, ${
    new Date().getFullYear() - car.year
  } years old, type: ${car.type[0].toUpperCase() + car.type.slice(1)}.`;
};
